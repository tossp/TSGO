...
func Schedule(spec string, job core.Job) error {
	sched, err := core.ParseStandard(spec)
	if err != nil {
		return err
	}
...
func Every(duration time.Duration, job core.Job) {

	MainCron.Schedule(core.Every(duration), New(job))
}

// Run the given job right now.
func Now(job core.Job) {
	go New(job).Run()
}

// Run the given job once, after the given delay.
func In(duration time.Duration, job core.Job) {
	go func() {
		time.Sleep(duration)
		New(job).Run()
	}()
