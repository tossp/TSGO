...
var ErrInvalidURLByScheme = errors.New("无效的URL(scheme)")
var ErrInvalidURLByType = errors.New("无效的URL(type)")
...
func NewKeyFromURL(orig string) (*Key, error) {
	...
	if u.Scheme != "otpauth" {
		return nil, ErrInvalidURLByScheme
	}
	u.Host = strings.ToLower(u.Host)
	if u.Host != "hotp" && u.Host != "totp" {
		return nil, ErrInvalidURLByType
	}
	return &Key{
		orig: s,
		url:  u,
...
// Algorithm returns a tiny int representing the rotation time in seconds.
...
// Digits returns a tiny int representing the rotation time in seconds.
